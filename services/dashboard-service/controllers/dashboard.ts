...
type LegendPayload = Map<string, boolean> | Record<string, boolean> | undefined;

const normalizeLegendPayload = (legend?: LegendPayload) => {
  if (!legend) {
    return undefined;
  }
...
const resolveClientId = async (req: Request): Promise<string | null> => {
  // @ts-ignore
  const existingClient: string | undefined = req.user?.client;
  if (existingClient) {
    return existingClient;
  }

  const headerClientRaw =
    req.headers["x-client-id"] ?? req.headers["client-id"] ?? req.headers["client"];
  const headerClient = Array.isArray(headerClientRaw)
    ? headerClientRaw[0]
    : headerClientRaw;
...
export const getWidgets = async (req: Request, res: Response): Promise<any> => {
  try {
    // @ts-ignore
    const userId = req.user._id;
...
export const createWidget = async (req: Request, res: Response): Promise<any> => {
  try {
    // @ts-ignore
    const userId = req.user._id;
    const client = await resolveClientId(req);
...
export const updateWidget = async (req: Request, res: Response): Promise<any> => {
  try {
    // @ts-ignore
    ...
    if (dataSourceParams !== undefined) widget.dataSourceParams = dataSourceParams;
...
export const deleteWidget = async (req: Request, res: Response): Promise<any> => {
  try {
    // @ts-ignore
    const userId = req.user._id.toString();
    const client = await resolveClientId(req);
...
export const reorderWidgets = async (req: Request, res: Response): Promise<any> => {
  try {
    // @ts-ignore
    const userId = req.user._id.toString();
    const client = await resolveClientId(req);
    ...
    const { widgetIds } = req.body as { widgetIds?: string[] };
...
export const getWidgetData = async (req: Request, res: Response): Promise<any> => {
  try {
    // @ts-ignore
    const userId = req.user._id.toString();
    const client = await resolveClientId(req);
