...
const STATISTICS_SERVICE_URL =
  process.env.STATISTICS_SERVICE_URL?.replace(/^"|"$/g, "") || "http://localhost:5003";
...
export async function fetchWidgetDataFromStatistics(widgetType: string, jwt?: string) {
  try {
    ...
    const message =
      error?.response?.data?.message || error?.message || "Statistics service unavailable";
